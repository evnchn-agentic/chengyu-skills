...
**Cultural-handle skills for Claude Code.** Each skill is a four-character idiom written in shared Han characters (漢字) — a Chinese chengyu (成語), a Japanese 四字熟語, or a Korean Hanja idiom of the same form — that names a specific strategic schema the model has seen contextualised thousands of times in training. Dropping the handle into a prompt — or letting the skill auto-trigger on description match — activates a tight constellation of behaviours that would otherwise take a paragraph to specify cold.
...
**The core = the natural-selection winners + the operative dependencies a core skill actually invokes:**
...
**Composition rides on top.** Task-skills that *compose* several of these — e.g. a transcript prompt-injection audit assembled from `listen-all-sides` plus the **non-core** `chengyu-two-pipes-adversarial` — ship as opt-in PRs layered over the core, never as always-on weight.
...
- **Lock to the four-character-idiom form, not to one nation.** Chinese chengyu (成語) are the heart of the catalog and its namesake; Japanese 四字熟語 and Korean Hanja idioms of the same form — same characters, same compressed-circuit mechanism — are equally welcome, no outlier status. What stays out, in sibling repos, is *other* handle traditions: Aesop fables, named-operator-personas, software design patterns. "Bucket by source" means bucket by register/mechanism (the four-character idiom the model has deeply internalised), not by alphabet — and not by passport. (The `chengyu-` directory prefix is kept as the catalog's namespace even for non-Chinese entries.)
- **The handle's canonical form must be written in shared Han characters (漢字).** This is the common factor that gives every member of the CJK family a fair shot at reading it. Chinese chengyu qualify natively; Japanese 四字熟語 qualify via their kanji form (一期一會, **not** いちごいちえ); Korean idioms qualify via Hanja (有備無患, **not** 유비무환). Forms that lock out part of the family are out — kana-only, Hangul-only, or single-topolect graphs (e.g. Cantonese-only characters). Romaji / kana / pinyin may ride along as a *reading annotation*, never as the handle itself. Rule of thumb: if it's a readable entry in [CJK Unified Ideographs](https://en.wikipedia.org/wiki/CJK_Unified_Ideographs), it's fair game.
