...
**Cultural-handle skills for Claude Code.** Each chengyu (Chinese four-character idiom) is one Claude Code skill that names a specific strategic schema the model has seen contextualised thousands of times in training. Dropping the handle into a prompt — or letting the skill auto-trigger on description match — activates a tight constellation of behaviours that would otherwise take a paragraph to specify cold.
...
Each SKILL.md embeds: handle (Traditional + Simplified characters), pinyin, schema description, "when this applies" symptoms, explicit behaviours, anti-pattern, and a brief etymology.
...
- A few skills ship as deliberate exceptions to the "lock to chengyu" rule, each with explicit rationale in the SKILL.md:
...
- **Lock to chengyu specifically.** This repo doesn't mix in Aesop, named-operator-personas, or design patterns. Bucket by source, not by alphabet. Those belong in sibling repos.
